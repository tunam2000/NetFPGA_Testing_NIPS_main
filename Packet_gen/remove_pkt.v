...
//     input                              out_rdy_11,
   ...
   if(DATA_WIDTH==32) begin:output_fifo32
      for(i=0; i<4; i=i+1) begin: output_fifos
   ...
   endgenerate
	  ...
	  generate
   genvar j;
   if(DATA_WIDTH==32) begin:output_fifo32_2
      for(j=8; j<10; j=j+1) begin: output_fifos_2
         syncfifo_512x36 gmac_tx_fifo
           (
            .clk         (clk),
            .din         (rd_0_data),
            .wr_en       (output_fifo_wr_en[j]),

            .dout        (output_fifo_dout[j]),
            .rd_en       (output_fifo_rd_en[j]),
   ...
   else if(DATA_WIDTH==64) begin: output_fifo64_2
      for(j=8; j<10; j=j+1) begin: output_fifos_2
         // we only need 256x72, but since we are going to
         // use 2 brams because of datawidth anyway...
         syncfifo_512x72 gmac_tx_fifo
